...
def getTodaysReport():
    ...
    for sale in sales:
        if sale["date_added"].date() == dateToday.date(): 
            ...
            sale=ItemsSoldPerSaleHistory.objects.filter(sale_history_id=sale["id"]).values("product_sold", "quantity_sold","price", "buying_price")
            for item in sale:
    ...
    itemsSoldTodaySet=set(map(lambda product:product["id"], itemsSoldToday)) # set of products sold today
    for item in itemsSoldTodaySet:
        ...
        product['total_value_sold']=product['profit'] * product['quantity_sold']
        finalReport.append(product)

    #create csv file of report and send email of the report
    fileName="Chic_favs_today's_Sale_report_as_of"+genDateTimeString(timezone.now())+".csv"
    fieldNames=["Product ID", "Product name", "Buying price","Selling price", "Quantity sold", "Profit","Total value earned"]
    formattedReport=[]
    for product in finalReport:
        product=dict(zip(fieldNames, product.values()))
        formattedReport.append(product)

    with open("pos/static/pos/documents/"+fileName, "w") as reportCsv:
    ...
    #send email
    with open("pos/static/pos/documents/"+fileName, "rb") as csvReport:
        subject="Chic favs today's sales report as of "+genDateTimeString(timezone.now())
        message="Hello. Hope this email finds you well.\nPlease find attached today's, {}, sales report for more analysis.".format(dateToday.strftime("%A, %d %B %Y"))
...
def getMonthReport():
    ...
    itemsSoldTodaySet=set(map(lambda product:product["id"], itemsSoldToday)) # set of products sold month
    for item in itemsSoldTodaySet:
        product_id=0
        quantitySold=[]
        name=""
    ...
    fileName="Chic_favs_{}_{}_sales report.csv".format(dateToday.strftime("%B"), dateToday.year)
    fieldNames=["Product ID", "Product name", "Buying price","Selling price", "Quantity sold", "Profit","Total value earned"]
    formattedReport=[]
    for product in finalReport:
        product=dict(zip(fieldNames, product.values()))
    ...
    #send email
    with open("pos/static/pos/documents/"+fileName, "rb") as csvReport:
        subject="Chic favs {} {} sales report".format(dateToday.strftime("%B"),dateToday.year)
        message="Hello. Hope this email finds you well.\nPlease find attached {} {}'s sales report for more analysis.".format(dateToday.strftime("%B"),dateToday.year)
        origin=settings.EMAIL_HOST_USER
        to=["adrianmuthomie@gmail.com"]
        email=EmailMessage(subject, message, origin, to)
        email.attach(fileName, csvReport.read(), 'text/csv')
...
def getYearReport():
    ...
    for sale in sales:
        if sale["date_added"].year == dateToday.year: 
            ...
            for item in sale:
                ...
                item['id']=productDetails.id
                item["name"]=productDetails.name
                itemsSoldToday.append(item) # Contains all items sold this year
    itemsSoldTodaySet=set(map(lambda product:product["id"], itemsSoldToday)) # set of products sold this year
    for item in itemsSoldTodaySet:
        product_id=0
        quantitySold=[]
        name=""
    ...
    with open("pos/static/pos/documents/"+fileName, "rb") as csvReport:
        ...
        message="Hello. Hope this email finds you well.\nPlease find attached {}'s sales report for more analysis.".format(dateToday.year)
        origin=settings.EMAIL_HOST_USER
        to=["adrianmuthomie@gmail.com"]
        email=EmailMessage(subject, message, origin, to)
        email.attach(fileName, csvReport.read(), 'text/csv')
