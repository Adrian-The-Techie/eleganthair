...
class API:
    ...
    def apis(self):
        ...
        if self.data["apiid"] == "updateEmployeeLevel":
            ...
            employeeLevels = EmployeeLevel.objects.get(
                id=self.data["data"]["id"])
            serializer = EmployeeLevelSerializer(
                instance=employeeLevels, data=data)
        ...
        if self.data["apiid"] == "getAllEmployees":
            ...
            for i in serializer.data:
                emp_level_details = EmployeeLevel.objects.get(
                    pk=i["emp_level"])
                emp_branch_details = Branch.objects.get(pk=i["branch"])

                serializedLevelDetails = EmployeeLevelSerializer(
                    emp_level_details, many=False)
                serializedBranchDetails = BranchSerializer(
                    emp_branch_details, many=False)
        ...
        if self.data["apiid"] == "updateEmployeeDetails":
            ...
            employeeInstance = Employees.objects.get(
                id=self.data["data"]["id"])
            serializer = EmployeesSerializer(
                instance=employeeInstance, data=self.data["data"])
        ...
        if self.data["apiid"] == "searchProducts":
            matchedProducts = Products.objects.filter(
                name_icontains=self.data["data"]["searchString"])
        ...
        if self.data["apiid"] == "updateProductDetails":
            ...
            serializer = ProductsSerializer(
                instance=productInstance, data=data)
        ...
        if self.data["apiid"] == "getSaleHistory":
            sales = SaleHistory.objects.values(
                "id", "customer_phone_number", "date_added")
        ...
        if self.data["apiid"] == "getSpecificSaleHistoryDetails":
            specificSaleHistory = SaleHistory.objects.get(
                id=self.data["data"]["id"])
            specificSaleHistory.date_added = genDateTimeString(
                dateObj=specificSaleHistory.date_added)
            serializedSaleHistory = SaleHistorySerializer(
                specificSaleHistory, many=False)
            ...
            items = ItemsSoldPerSaleHistory.objects.filter(sale_history_id=values["id"]).values(
                "id", "product_sold_id", "price", "quantity_sold", "sub_total")
        ...
        if self.data["apiid"] == "resendReceipt":
            ...
            for item in self.data["data"]["items"]:
                itemsString += item["product_sold_id"]+", "+str(item["quantity_sold"])+" piece(s), "+str(
                    item["price"])+" per piece, sub-total is "+str(item["sub_total"])+"\n"

            message = "Here is your receipt;\nReceipt ID: "+str(self.data["data"]["receiptId"])+"\n"+itemsString+"\n Sold by: " + \
                self.data["data"]["sold_by"]+"\n GRAND TOTAL: " + \
                self.data["data"]["amount_sold"] + \
        ...
        if self.data["apiid"] == "getUserAndProducts":
            salesPeople = Employees.objects.filter(
                emp_level=2).values("id", "username")
            products = Products.objects.values(
                "id", "name", "quantity_in_stock", "selling_price")
        ...
        if self.data["apiid"] == "getSpecificAllocationDetails":
            ...
            availableItems = ItemsAllocated.objects.filter(
                product_allocated_to=user_id).values("product_allocated", "quantity_allocated")
            allocationHistory = AllocationHistory.objects.filter(
                emp_allocated_to=user_id).values("id", "date_allocated")

            for item in availableItems:
                itemInstance = Products.objects.get(
                    id=item["product_allocated"])
                item["product_allocated"] = itemInstance.name

            for specificHistory in allocationHistory:
                specificHistory["date_allocated"] = genDateTimeString(
                    specificHistory["date_allocated"])
        ...
        if self.data["apiid"] == "getSpecificAllocationHistoryDetails":
            specificAllocationHistory = AllocationHistory.objects.get(
                id=self.data["data"]["id"])
            specificAllocationHistory.date_allocated = genDateTimeString(
                dateObj=specificAllocationHistory.date_allocated)
            serializedAllocationHistory = AllocationHistorySerializer(
                specificAllocationHistory, many=False)
            ...
            items = ItemsAllocatedHistory.objects.filter(
                allocation_history=values["id"]).values("id", "product", "quantity_allocated")
            values["items_sold"] = items
            for specificItem in values["items_sold"]:
                ...
                specificItem["sub_total_value"] = specificItem["selling_price"] * \
                    specificItem["quantity_allocated"]
            # get grand total
            subTotals = list(
                map(lambda item: item["sub_total_value"], values["items_sold"]))
            values["grand_total_value"] = functools.reduce(
                operator.add, subTotals) if len(subTotals) > 0 else 0
        ...
        if self.data["apiid"] == "getSellingProducts":
            ...
            if self.data["data"]["type_of_sale"] == 1:
                products = Products.objects.values(
                    "id", "name", "selling_price", "quantity_in_stock", "buying_price")

            else:
                products = ItemsAllocated.objects.filter(product_allocated_to=self.data["data"]["user_id"]).values(
                    "id", "product_allocated", "quantity_allocated")
                for product in products:
                    productInstance = Products.objects.get(
                        id=product["product_allocated"])
        ...
        if self.data["apiid"] == "performStockTake":
            productsQuantityInStock = Products.objects.values(
                "id", "name", "quantity_in_stock", "selling_price")
            subTotalValueList = []

            for product in productsQuantityInStock:
                itemAllocatedQuantityInStock = ItemsAllocated.objects.filter(
                    product_allocated=product["id"]).values("quantity_allocated")
                product["item_allocated_quantity_in_stock"] = itemAllocatedQuantityInStock
                quantity_in_stock = map(
                    lambda product: product["quantity_allocated"], product["item_allocated_quantity_in_stock"])
                product["item_allocated_quantity_in_stock"] = quantity_in_stock
                product["item_allocated_quantity_in_stock"] = functools.reduce(
                    operator.add, product["item_allocated_quantity_in_stock"], 0)
                product["total_quantity_in_stock"] = product["quantity_in_stock"] + \
                    product["item_allocated_quantity_in_stock"]
                product["sub_total_value"] = product["total_quantity_in_stock"] * \
            ...
            fieldNames = ["Product ID", "Product Name", "Quantity in stock for normal sale", "Selling price",
                          "Quantity in stock for allocation sale", "Total quantity in stock", "Total valuation of product"]
            formattedData = []
            ...
            # save
                stockTakeHistoryInstance = StockTakeHistory(
                    csv_file=finalFile.name)
                stockTakeHistoryInstance.save()

                finalFile.close()
            # email
            # try:
            with open("ChicFavs/static/ChicFavs/documents/"+fileName, "rb") as report:
        ...
        if self.data["apiid"] == "getStockTakeHistories":
            stockTakeHistories = StockTakeHistory.objects.values(
                "id", "date_added")
            for history in stockTakeHistories:
                history["date_added"] = genDateTimeString(
                    history["date_added"])
        ...
        if self.data["apiid"] == "getSpecificStockTakeHistory":
            stockTakeHistoryInstance = StockTakeHistory.objects.get(
                id=self.data["data"]["id"])
            csvFile = stockTakeHistoryInstance.csv_file.name
            dictKeys = ["id", "name", "quantity_in_stock", "selling_price",
                        "item_allocated_quantity_in_stock", "total_quantity_in_stock", "sub_total_value"]
            products = []
            formattedProducts = []
            subTotalValueList = []
            ...
            with open(csvFile, "r") as report:
                ...
                for line in csvReader:
                    ...
                    line["Quantity in stock for normal sale"] = int(
                        line["Quantity in stock for normal sale"])
                    line["Selling price"] = float(line["Selling price"])
                    line["Quantity in stock for allocation sale"] = float(
                        line["Quantity in stock for allocation sale"])
                    line["Total quantity in stock"] = float(
                        line["Total quantity in stock"])
                    line["Total valuation of product"] = float(
                        line["Total valuation of product"])
